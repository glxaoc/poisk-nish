...
async function updateQuota(){
  try{
    var r=await fetch('/api/userInfo',{method:'POST',headers:{'Content-Type':'application/json'},body:'{}'});
    var j=await r.json();
    var i=j.userInfo||{};
    document.getElementById('qr').innerText=i.dailyLimitRemaining||'-';
    document.getElementById('ql').innerText=i.dailyLimit||'-';
    log('Quota: '+i.dailyLimitRemaining+'/'+i.dailyLimit,'ok');
  }catch(e){log('Quota error: '+e,'err')}
}

async function doQuota(){
  render('<div class="loading">Загрузка...</div>');
  log('POST /api/userInfo','info');
  try{
    var r=await fetch('/api/userInfo',{method:'POST',headers:{'Content-Type':'application/json'},body:'{}'});
    var j=await r.json();
    ...
    document.getElementById('qr').innerText=i.dailyLimitRemaining||'-';
    document.getElementById('ql').innerText=i.dailyLimit||'-';
    render('<h3>Квота</h3><p>Логин: '+i.login+'</p><p>Осталось: '+i.dailyLimitRemaining+' из '+i.dailyLimit+'</p>');
  }catch(e){render('<div class="error">'+e+'</div>')}
}

async function doTop(){
  var p=document.getElementById('phrase').value;
  var reg=parseInt(document.getElementById('region').value);
  render('<div class="loading">Загрузка...</div>');
  log('POST /api/topRequests','info');
  try{
    var r=await fetch('/api/topRequests',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({phrase:p,region:reg})});
    var d=await r.json();
    log('Status: '+r.status+', Keys: '+Object.keys(d).join(','),'ok');
    if(d.error){render('<div class="error">'+d.error+'</div>');return}
    var items=d.topRequests||[];
    var assoc=d.associations||[];
    log('Items: '+items.length+', Assoc: '+assoc.length,'ok');
    var h='<h3>Топ запросов: "'+p+'"</h3>';
    h+='<div class="stat-box"><div class="value">'+fmt(d.totalCount||0)+'</div><div>Всего запросов</div></div>';
    h+='<table><thead><tr><th>#</th><th>Запрос</th><th>Частота</th></tr></thead><tbody>';
    for(var i=0;i<Math.min(items.length,20);i++){h+='<tr><td>'+(i+1)+'</td><td>'+items[i].phrase+'</td><td class="count">'+fmt(items[i].count)+'</td></tr>';}
    h+='</tbody></table>';
    if(assoc.length>0){
      h+='<h3 style="margin-top:25px">Похожие запросы</h3><table><thead><tr><th>#</th><th>Запрос</th><th>Частота</th></tr></thead><tbody>';
      for(var i=0;i<Math.min(assoc.length,10);i++){h+='<tr><td>'+(i+1)+'</td><td>'+assoc[i].phrase+'</td><td class="count">'+fmt(assoc[i].count)+'</td></tr>';}
      h+='</tbody></table>';
    }
    render(h);
    updateQuota();
...
async function doDyn(){
  var p=document.getElementById('phrase').value;
  var reg=parseInt(document.getElementById('region').value);
  render('<div class="loading">Загрузка...</div>');
  log('POST /api/dynamics','info');
  try{
    var r=await fetch('/api/dynamics',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({phrase:p,region:reg})});
    var d=await r.json();
    log('Status: '+r.status,'ok');
    if(d.error){render('<div class="error">'+d.error+'</div>');return}
    var items=d.dynamics||[];
    log('Dynamics: '+items.length+' items','ok');
    ...
    for(var i=0;i<items.length;i++){
      ...
      labels.push(months[m]+' '+dt.slice(0,4));
      data.push(items[i].count);
    }
    
    var h='<h3>Динамика: "'+p+'"</h3>';
    h+='<div class="chart-container"><canvas id="dynChart"></canvas></div>';
    h+='<table><thead><tr><th>Месяц</th><th>Запросов</th><th>Доля</th></tr></thead><tbody>';
    for(var i=0;i<items.length;i++){h+='<tr><td>'+items[i].date.slice(0,7)+'</td><td class="count">'+fmt(items[i].count)+'</td><td>'+(items[i].share*100).toFixed(2)+'%</td></tr>';}
    h+='</tbody></table>';
    render(h);
...
async function doReg(){
  var p=document.getElementById('phrase').value;
  render('<div class="loading">Загрузка...</div>');
  log('POST /api/regions','info');
  try{
    var r=await fetch('/api/regions',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({phrase:p})});
    var d=await r.json();
    log('Status: '+r.status,'ok');
    if(d.error){render('<div class="error">'+d.error+'</div>');return}
    var items=d.regions||[];
    log('Regions: '+items.length+' items','ok');
    var h='<h3>Регионы: "'+p+'"</h3><table><thead><tr><th>#</th><th>Регион</th><th>Запросов</th><th>Доля</th></tr></thead><tbody>';
    for(var i=0;i<Math.min(items.length,20);i++){
      var name=getRegionName(items[i].regionId);
      h+='<tr><td>'+(i+1)+'</td><td>'+name+'</td><td class="count">'+fmt(items[i].count)+'</td><td>'+(items[i].share*100).toFixed(2)+'%</td></tr>';
    }
    h+='</tbody></table>';
    render(h);
    updateQuota();
...
window.onerror=function(m,u,l){log('JS: '+m+' line '+l,'err')};
log('Ready','ok');
updateQuota();
</script>
</body>
...
def api_dynamics():
    try:
        data = request.json or {}
        payload = {"phrase": data.get("phrase", ""), "period": "monthly", "fromDate": "2025-01-01", "toDate": "2025-11-30"}
        region = data.get("region", 0)
        if region and region != 0:
            payload["regions"] = [region]
